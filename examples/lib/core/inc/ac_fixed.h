...
/*-----------------------------------------------------------------------------
 * ADDED TO SUPPORT "HALCON" PROJECT REQUIREMENTS (BEGIN)
 *-----------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------
 * ADDED TO SUPPORT "HALCON" PROJECT REQUIREMENTS (END)
 *-----------------------------------------------------------------------------*/
...
/* ADDED THIS ABSTRACT BASE CLASS TO BE ABLE TO REFLECT */
/* Goal is to be able to read the value as int, and to be able to set the value
from string (i.e. using command handler or yaml setting)*/
class AbstractACFixed
{
public:
  AbstractACFixed() = default;
  virtual ~AbstractACFixed() = default;

  /**
   * @brief Get variable width in bits
   * 
   * @return int 
   */
  ...
  /**
   * @brief Get the number of bits in integer part representation
   * 
   * @return int 
   */
  virtual inline int GetI() const = 0;
  
  /**
   * @brief Get the number of bits in fractional part representation
   * 
   * @return int 
   */
  virtual inline int GetF() const = 0;

  /**
   * @brief Get the equivalent decimal representation
   * 
   * @return int 
   */
  virtual inline int GetDecValue() const = 0;
   ...
   * @return double 
   */
  virtual inline double Get() const = 0;
   ...
   * @param value 
   */
  virtual inline void Set(std::string value) = 0;

  /**
   * @brief Set the value from double
   * 
   * @param value 
   */
  virtual inline void Set(double value) = 0;
};



template<int W, int I, bool S=true, ac_q_mode Q=AC_TRN, ac_o_mode O=AC_WRAP>
class ac_fixed : private ac_private::iv<(W+31+!S)/32>, public AbstractACFixed
#ifndef __SYNTHESIS__
...
#pragma clang diagnostic pop
  ...
  /* ADDED THESE METHODS TO REFLECT */
  inline int GetW() const { return W; }
  inline int GetI() const { return I; }
  inline int GetF() const { return W-I; }
  inline int GetDecValue() const {return to_double()*pow(2, GetF());}
  inline double Get() const {return to_double();}
  inline void Set(std::string value) {(*this) = std::stod(value);}
  inline void Set(double value) {(*this) = value;}
