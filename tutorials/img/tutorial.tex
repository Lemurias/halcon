...
\begin{frame}{Tutorial example}
	...
	\node[node_output, right of = fir_filter, node distance = 2 cm, label={30:o\_signal}] (o_signal){};
	\draw[arrows=->,>=latex] (fir_filter) -- (o_signal);

	\node[square_block, right of = o_signal , node distance = 4cm]  (file_o){file};
	\draw[dotted,arrows=->,>=latex] (o_signal) -- (file_o);
