...
\begin{frame}{TP5}
	\begin{figure}
		...
		\begin{tikzpicture}[thick,scale=0.6, every node/.style={scale=0.6}]
			...
			\node[block, right of = upsampler, node distance = 3.5 cm, label={u\_rc\_filter}] (rc_filter){Raised Cosine Filter};
			\node[node_output, right of = rc_filter, node distance = 3 cm, label = {0:o\_signal}] (o_signal) {} ;
			
			\draw[arrows=->, >=latex] (symbolgenerator) -- (upsampler);
			\draw[arrows=->, >=latex] (upsampler) -- (rc_filter);
			\draw[arrows=-, >=latex] (rc_filter) -- (o_signal);
...
\begin{frame}{Raised Cosine Filter}
    \begin{figure}[h!]
        \centering
...
%            \draw[dotted, arrows=->,>=latex] (aux_clk_1.center) -- ++(0,3) -| (reg_2);
 %           \draw[dotted, arrows=->,>=latex] (aux_clk_1.center) -- ++(0,3) -| (reg_3);
        ...
        \end{tikzpicture}
        \caption{Raised Cosine Filter Module with 24 taps}
        \label{fig:lab_02}
    \end{figure}
\end{frame}
